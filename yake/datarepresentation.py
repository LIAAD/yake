...
class DataCore():
    ...
    def _build(self, text, windows_size, n):
        ...
        for (sentence_id, sentence) in enumerate(self.sentences_str):
            pos_text = self.process_sentence(
                sentence, sentence_id, pos_text, windows_size, n)
    ...
    def build_single_terms_features(self, features=None):
        valid_terms = [term for term in self.terms.values()
    ...
    def get_term(self, str_word, save_non_seen=True):
        ...
        # until here
        isstopword = simples_sto or unique_term in self.stopword_set or len(
            simples_unique_term) < 3
            ...
            if params.get('rel_approx', True):
                ...
                for gold_key in params['keys']:
                    dist = 1.-jellyfish.levenshtein_distance(
                        # _tL
                        gold_key, self.unique_kw) / max(len(gold_key), len(self.unique_kw))
...
class SingleWord():
    ...
    def update_h(self, stats, features=None):
        ...
        if features is None or "wpos" in features:
            self.wpos = math.log(
                math.log(3. + np.median(list(self.occurs.keys()))))
